...
int d_push_back_vector(dVector *vector, void *content)
{
    ...
    else if (vector->size == vector->real_size)
    {
        vector->real_size *= 2;
        if (vector->content_array == NULL)
            return DSTRUCT_VECTOR_ARRAY_NULL;
    ...
    (vector->size)++;
    return d_insert_vector(vector, vector->size - 1, content);
}
...
int d_insert_vector(dVector *vector, unsigned int i, void *content)
{
    ...
    if (i >= vector->size)
        return DSTRUCT_NOT_IN_VECTOR;
    else if (vector->content_array[i] != NULL)
        return DSTRUCT_VECTOR_SPACE_OCCUPIED;
    else
