...
int d_push_heap(dHeap *heap, void *content)
{
    if(content == NULL)
...
int d_clear_heap(dHeap *heap, void (*free_content)(void *))
{
...
{
    dNode *tmp;
    if (top_node->left == bottom_node)
    { // bottom_node is on the left
        // right
...
int go_up(dHeap *heap, dNode *node)
{
    if ((node->prev != NULL) && heap->sort_test((void *)node->content, (void *)node->prev->content))
    { // so node go_up
